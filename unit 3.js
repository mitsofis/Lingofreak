        ...
        function checkExercise3() {
            ...
            document.querySelectorAll('.sentence-drop').forEach(dropZone => {
                ...
                const userSentence = Array.from(dropZone.children)
                    ...
                    .replace(/\s+/g, ' ')
                    .trim();
                console.log(`Sentence ${sentenceId} - User: "${userSentence}", Expected: "${answers[sentenceId]}"`);
                if (userSentence === answers[sentenceId]) {
                    correct++;
                    feedback += `<li>Sentence ${sentenceId}: Correct!</li>`;
                } else {
                    feedback += `<li>Sentence ${sentenceId}: Incorrect. Correct sentence: "${answers[sentenceId]}". Your answer: "${userSentence}".</li>`;
                }
            });
            feedback += '</ul>';
